...
func setupRouter(r *mux.Router) {
	accLogRepo := repository.NewAccessLogRepoDB(database.DB)
	scoreSrv := services.NewDefaultScoreService(accLogRepo)
	todoHandler := handlers.NewScoreHandler(scoreSrv)
	newspews := r.PathPrefix("/api/newspews").Subrouter()
	newspews.HandleFunc("", todoHandler.CreateTodo).Methods(http.MethodPost)
